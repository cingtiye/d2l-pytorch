...
def draw_text(img: str, texts: list, locations: list) -> np.ndarray:
    for text, loc in zip(texts, locations):
        cv2.putText(img, text, (int(loc[0] - (loc[2] / 2) - 5), int(loc[1] - (loc[3] / 2) - 5)),
...
class PIKACHU(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = np.array(
            Image.open(image_path).convert('RGB').resize((self.image_size[2], self.image_size[1]), Image.BILINEAR))
        # print(img.shape)
